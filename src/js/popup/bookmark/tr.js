    ...
    render() {
        ...
        const menu = (row) => (
            ...
            <Menu style={{width: 200}}>
                <Menu.SubMenu title="bookmark search">
                ...
                <Menu.SubMenu title="edit bookmark">
                    <Menu.Item>
                        <a size="small"
                           onClick={deleteItem.bind(this, row, this.deleteCallback.bind(this, row))}>delete</a>
                    </Menu.Item>
                    <Menu.Item>
                        <a size="small"
                           onClick={() => _this.setState({update: true}, () => this.setState({edit: true}, () => {
                ...
                <Menu.SubMenu title="Google Search">
                    <Menu.Item>
                        <a target="_blank" href={`https://www.google.com/search?q=site:` + getDomain(row.url)}><span
                            style={{marginRight: "1em"}}><Icon type="google"/></span>site search</a>
                    </Menu.Item>
                    <Menu.Item>
                        <a target="_blank" href={`https://www.google.com/search?q=intitle:` + row.title}><span
                            style={{marginRight: "1em"}}><Icon type="google"/></span>intitle search</a>
                    </Menu.Item>
                    <Menu.Item>
                        <a target="_blank" href={`https://www.google.com/search?q=inurl:` + row.url}><span
            ...
            <tr>
                <td ref={(dom) => {
                    ...
                    <div draggable="true" ref={dom => {
                        ...
                        {!edit && <a style={{marginRight: "2em"}} target="_blank" href={row.url}
                                     onClick={this.handleClick.bind(self, row)}>{row.url && <span style={{
                            width: 16,
                            height: 16,
                            backgroundImage: `-webkit-image-set(url("chrome://favicon/size/16@1x/${row.url}") 1x, url("chrome://favicon/size/16@2x/${row.url}") 2x)`
                        }} className="img"></span> || <Icon className="img" type="folder" theme="outlined"/>}
                            <span className="wy_title"
                                  dangerouslySetInnerHTML={{__html: newTitle || splitTitle(title).title}}></span>
                            {!row.url && <span>&nbsp;({row.children.length})</span>}
                        </a> || <div style={{marginRight: "2em"}}>
                            <Input ref="input" style={{lineHeight: 2, padding: '0.3em'}} size="small" value={title}
                                   onChange={(e) => this.setState({title: e.target.value})}
                                   onPressEnter={this.cancleEdit.bind(this, row)}
                                   onBlur={this.cancleEdit.bind(this, row)}/>
                        </div>}
